...
/**
 * Retrieve all table names from the current database that begin with a specific prefix.
 *
 * @param string         $table_prefix The prefix to filter table names (e.g. tenant_123_).
 * @param CI_DB_mysqli   $db           The CodeIgniter DB instance to use for querying.
 *
 * @return array                      An array of table names matching the prefix.
 */
function perfex_saas_get_db_tables_with_prefix($table_prefix, $db)
{
    // Query the database for tables starting with the prefix
    $query = $db->query("
...
function perfex_saas_recurring_invoice_will_cancel($invoice, $client_id = null)
{
    static $cache = null;
    if ($cache && isset($cache[$invoice->id])) {
        return $cache[$invoice->id];
    }

    $client_id = $client_id ?? get_client_user_id();
    $promised_cancel_recurring_invoice = perfex_saas_get_or_save_client_metadata($client_id)['will_cancel_invoice_at_period_end'] ?? '';
    $promised_to_cancel = empty($invoice->stripe_subscription_id) && $promised_cancel_recurring_invoice == $invoice->id;
    $cache[$invoice->id] = $promised_to_cancel;

    return $promised_to_cancel;
}