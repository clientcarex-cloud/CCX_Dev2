    ...
    try {
        if (isset($_SERVER['REQUEST_URI'])) {
            ...
            $options = perfex_saas_get_options(
                ...
                true,
                " OR ( `name` LIKE '%ps!_global%' ESCAPE '!' ) " // global cross load option should use ps_global in settings name
            );
 ...
 * @param array $content An array containing extra context to the error (optional)
 */
function perfex_saas_show_tenant_error($heading, $message, $error_code = 403, $template = '404', $tag = '', $context = [])
{
 ...
 * @param string $extra_clause  The extra where clause for the query
 * @return mixed            The shared secret master settings.
 * @Bridge-Function
 */
function perfex_saas_get_options($field, bool $parse = true, $extra_clause = '')
{
    $single_option = is_string($field);
    $fields = "'" . implode("','", $single_option ? [$field] : $field) . "'";
    $option_query = 'SELECT name, value FROM ' . perfex_saas_master_db_prefix() . "options WHERE `name` IN ($fields) $extra_clause";
