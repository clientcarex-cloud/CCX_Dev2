(function($) { 
  ...
  const fnSetupPeriod = async function () {
    var report_from = $(
      '.ccxdashboard-period-select input[name="period_from"]'
    );
    var report_to = $('.ccxdashboard-period-select input[name="period_to"]');
    var date_range = $(".ccxdashboard-date-range");

    $(".ccxdashboard-period-dropdown").on("change", function () {
      var val = $(this).val();
      var val_to = "";
      var val_from = "";
    ...
    $(".ccxdashboard-period-dropdown").trigger("change");
  };
  ...
  const fnGetLang = async function (langs) {
    return new Promise((resolve, reject) => {
      ...
      $.ajax({
        url: site_url + "admin/ccx_dashboard/api/lang",
        type: "GET",
        data: reqData,
        dataType: "json",
        success: function (rs) {
  ...
  const fnDashboardJs = async function () {
    if (
      typeof window.ccx_dashboard_enable_sortable !== "undefined" ||
      window.ccx_dashboard_enable_sortable
    ) {
      var langs = await fnGetLang(["hide_widget_area", "widget_area"]);

      window.ccx_dashboard_update_widgets_order = function () {
        var order = {};
        ...
        $.ajax({
          url:
            site_url +
            "admin/ccx_dashboard/dashboards/api_update_widgets_order",
          type: "POST",
          data: reqData,
          dataType: "json",
          success: function (rs) {
        ...
        $("[data-container]").sortable({
          ...
          update: function (event, ui) {
            if (this === ui.item.parent()[0]) {
              window.ccx_dashboard_update_widgets_order();
            }
          },
        });
  ...
  const fnPageCategories = async function () {
    var langs = await fnGetLang(["ccx_dashboard_confirm_delete"]);
    ...
    //edit category
    var ccx_dashboard_lock_edit = false;
    $(".btn-edit-category").on("click", function () {
      if (ccx_dashboard_lock_edit) {
        return;
      }
      ...
      ccx_dashboard_lock_edit = true;
      $.ajax({
        url:
          site_url +
          "admin/ccx_dashboard/categories/api_get_category_data?category_id=" +
          id,
        type: "GET",
        data: {},
        dataType: "json",
    ...
    $(".btn-delete").on("click", function () {
      var id = $(this).data("id");
      if (id != undefined && id != null && id != "") {
        if (!confirm(langs.ccx_dashboard_confirm_delete)) {
          return;
        }
  ...
  const fnPageWidgets = async function () {
    ...
    if (widget_edit_permission) {
      $(".widget").prepend(
        `<div class="ccxdashboard-widget-removable"></div>`
      );
    }
    if (widget_delete_permission) {
      $(".widget").prepend(
        `<div class="ccxdashboard-widget-changeable"></div>`
      );
    }

    // on remove widget
    $("body").on("click", ".ccxdashboard-widget-removable", function () {
      if (!confirm(langs.ccx_dashboard_confirm_delete)) {
        return;
      }
    ...
    var ccx_dashboard_lock_changeable = false;
    $("body").on("click", ".ccxdashboard-widget-changeable", function () {
      if (ccx_dashboard_lock_changeable) {
        return;
      }
      ...
      ccx_dashboard_lock_changeable = true;
      $.ajax({
        url:
          site_url +
          "admin/ccx_dashboard/widgets/api_get_widget_data?widget_id=" +
          id,
        type: "GET",
        data: {},
        dataType: "json",
        success: function (rs) {
          ...
          ccx_dashboard_lock_changeable = false;
        },
        error: function () {
          ccx_dashboard_lock_changeable = false;
        },
  ...
  const fnPageEditDashboard = async function () {
    var langs = await fnGetLang([
      "add",
      "preview",
      "ccx_dashboard_confirm_delete",
      "not_found",
      "ccx_dashboard_message_success_delete_widget_from_dashboard",
    ]);

    window.ccx_dashboard_enable_sortable = true;
    fnDashboardJs();

    var widget_preview_url =
      site_url + "admin/ccx_dashboard/widgets?category=&search=";
    // add widget
    (function () {
      $("#formAddWidget").on(
        "click",
        ".ccxdashboard-add-widget",
        function () {
          var widgetId = $(this).data("widget-id");
    ...
    (function () {
          ...
          availables.forEach((available) => {
            wrapper.append(`
              <tr>
                ...
                <td><a class="ccxdashboard-add-widget" href="javascript:void(0);" data-widget-id="${
                  available.id
                }" data-widget-container="${available.default_container}">${
              langs["add"]
            }</a></td>
                <td><a class="ccxdashboard-add-widget" href="${
      ...
      window.ccx_dashboard_fetch_available_widgets = function () {
        var reqData = {
          dashboard_id: DASHBOARD_ID,
        };
        reqData[csrfData.token_name] = csrfData.hash;
        $.ajax({
          url:
            site_url +
            "admin/ccx_dashboard/dashboards/api_available_widgets",
          type: "POST",
          data: reqData,
          dataType: "json",
          success: function (rs) {
      ...
      window.ccx_dashboard_fetch_available_widgets();
    })();
    ...
    (function () {
      ...
      $("[data-container]").on(
        ...
        function () {
          ...
          alert_float(
            "warning",
            langs.ccx_dashboard_message_success_delete_widget_from_dashboard
          );
        }
      );
    })();
