...
// Rename legacy Perfex Dashboard tables to the new CCX Dashboard naming if they still exist.
$legacyTables = [
    'perfex_dashboard_widget_categories' => 'ccx_dashboard_widget_categories',
    'perfex_dashboard_list'              => 'ccx_dashboard_list',
    'perfex_dashboard_users'             => 'ccx_dashboard_users',
    'perfex_dashboard_widgets'           => 'ccx_dashboard_widgets',
];

foreach ($legacyTables as $oldTable => $newTable) {
    if ($CI->db->table_exists(db_prefix() . $oldTable) && !$CI->db->table_exists(db_prefix() . $newTable)) {
        $CI->db->query('RENAME TABLE ' . db_prefix() . $oldTable . ' TO ' . db_prefix() . $newTable);
    }
}

// Ensure admin route points to CCX dashboard after activation.
$routesPath = APPPATH . 'config/my_routes.php';
$routeSnippet = "if (file_exists(APPPATH . '../modules/ccx_dashboard/ccx_dashboard.php')) {\n    \$route['admin'] = 'admin/ccx_dashboard/dashboards/my_dashboard';\n}";
if (!file_exists($routesPath)) {
    // Create file with PHP open tag and the route snippet.
    $routeFileContents = "<?php\n\n{$routeSnippet}\n";
    @file_put_contents($routesPath, $routeFileContents);
} else {
    $existing = @file_get_contents($routesPath);
    if ($existing !== false && strpos($existing, "ccx_dashboard/dashboards/my_dashboard") === false) {
        // Append the snippet safely with a newline separator.
        $append = (substr($existing, -1) === "\n" ? '' : "\n") . $routeSnippet . "\n";
        @file_put_contents($routesPath, $existing . $append);
    }
}

// Migrate existing permission slugs to the new ccx_dashboard name.
if ($CI->db->table_exists(db_prefix() . 'staff_permissions')) {
    $CI->db->where('feature', 'perfex_dashboard');
    $CI->db->update(db_prefix() . 'staff_permissions', ['feature' => 'ccx_dashboard']);
}

if ($CI->db->table_exists(db_prefix() . 'permissions')) {
    $CI->db->where('shortname', 'perfex_dashboard');
    $CI->db->update(db_prefix() . 'permissions', [
        'shortname' => 'ccx_dashboard',
        'name'      => 'CCX dashboard',
    ]);
}

if (!$CI->db->table_exists(db_prefix() . 'ccx_dashboard_widget_categories')) {
  $CI->db->query("
    CREATE TABLE " . db_prefix() . "ccx_dashboard_widget_categories (
  ...
  $CI->db->query("
    ALTER TABLE " . db_prefix() . "ccx_dashboard_widget_categories
    ADD PRIMARY KEY (id);
  ");
  ...
  $CI->db->query("
    INSERT INTO " . db_prefix() . "ccx_dashboard_widget_categories (`id`, `name`, `note`) VALUES
...
if (!$CI->db->table_exists(db_prefix() . 'ccx_dashboard_list')) {
  $CI->db->query("
    CREATE TABLE " . db_prefix() . "ccx_dashboard_list (
    id                    BIGINT(20) NOT NULL,
    name                  VARCHAR(191) DEFAULT NULL,
  ...
  $CI->db->query("
    ALTER TABLE " . db_prefix() . "ccx_dashboard_list
    ADD PRIMARY KEY (id);
  ");
...
if (!$CI->db->table_exists(db_prefix() . 'ccx_dashboard_users')) {
  $CI->db->query("
    CREATE TABLE " . db_prefix() . "ccx_dashboard_users (
    id                    BIGINT(20) NOT NULL,
  ...
  $CI->db->query("
    ALTER TABLE " . db_prefix() . "ccx_dashboard_users
    ADD PRIMARY KEY (id);
  ");
  ...
  // Default dashboard
  $CI->db->query("
    INSERT INTO " . db_prefix() . "ccx_dashboard_list (`id`, `name`, `dashboard_widgets`, `note`, `updated_at`, `created_at`) VALUES
...
if (!$CI->db->table_exists(db_prefix() . 'ccx_dashboard_widgets')) {
  $CI->db->query("
    CREATE TABLE " . db_prefix() . "ccx_dashboard_widgets (
    id                    BIGINT(20) NOT NULL,
    name                  VARCHAR(191) DEFAULT NULL,
  ...
  $CI->db->query("
    ALTER TABLE " . db_prefix() . "ccx_dashboard_widgets
    ADD PRIMARY KEY (id);
  ");
  ...
  // Default widgets
  $CI->db->query("
    INSERT INTO " . db_prefix() . "ccx_dashboard_widgets (`name`, `category`, `widget_name`, `default_container`, `note`) VALUES
