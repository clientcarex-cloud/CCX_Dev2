...
$this->app_css->add('ccx_dashboard_styles', CCX_DASHBOARD_ASSETS_PATH . '/css/ccx_dashboard_styles.css');
$this->app_scripts->add('ccx_dashboard_scripts', CCX_DASHBOARD_ASSETS_PATH . '/js/ccx_dashboard_scripts.js');
$this->app_scripts->add('ccx_dashboard_widget_scripts', CCX_DASHBOARD_ASSETS_PATH . '/js/widgets/widget.js');
$this->app_css->add('ccx_dashboard_widget_styles', CCX_DASHBOARD_ASSETS_PATH . '/css/widget.css');
$categories = ccx_dashboard_get_categories();
...
?>
  ...
  <div id="wrapper">
    <div class="content">
      <div class="row">
        <div class="col-md-12">
          <form method="get" action="<?php echo admin_url('ccx_dashboard/dashboards/edit_dashboard'); ?>">
            <input type="hidden" name="dashboard_id" value="<?= $dashboard['id'] ?>" />
            <div class="ccxdashboard-actions-wrap ccxdashboard-actions-wrap-between">
              <div class="ccxdashboard-actions-section-left">
                <h4><?= _l('edit') ?> <?= $dashboard['name']; ?></h4>
              </div>
              <div class="ccxdashboard-actions-section-right"></div>
            </div>
          </form>
        </div>
      </div>
      ...
      <?php } else { ?>
        <div class="row mtop30">
          <div class="col-md-3" data-container="top-left-first-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'top-left-first-4'); ?>
          </div>
          <div class="col-md-3" data-container="top-left-last-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'top-left-last-4'); ?>
          </div>
          <div class="col-md-3" data-container="top-right-first-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'top-right-first-4'); ?>
          </div>
          <div class="col-md-3" data-container="top-right-last-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'top-right-last-4'); ?>
          </div>
        </div>

        <div class="row">
          <div class="col-md-12" data-container="top-12">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'top-12'); ?>
          </div>
        </div>

        <div class="row">
          <div class="col-md-6" data-container="middle-left-6">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'middle-left-6'); ?>
          </div>
          <div class="col-md-6" data-container="middle-right-6">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'middle-right-6'); ?>
          </div>
        </div>

        <div class="row">
          <div class="col-md-8" data-container="left-8">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'left-8'); ?>
          </div>
          <div class="col-md-4" data-container="right-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'right-4'); ?>
          </div>
        </div>

        <div class="row">
          <div class="col-md-4" data-container="bottom-left-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'bottom-left-4'); ?>
          </div>
          <div class="col-md-4" data-container="bottom-middle-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'bottom-middle-4'); ?>
          </div>
          <div class="col-md-4" data-container="bottom-right-4">
            <?php ccx_dashboard_render_widgets_from_dashboard($dashboard, 'bottom-right-4'); ?>
          </div>
        </div>
      <?php } ?>
      <div class="btn-bottom-toolbar btn-toolbar-container-out text-right">
        <a id="viewWidgetableArea" href="javascript:void(0);" class="btn btn-default"><?= _l('widget_area') ?></a>
        <?php if (has_permission('ccx_dashboard', '', 'dashboard_edit')) { ?>
        ...
        <?php } ?>
        <?php if (has_permission('ccx_dashboard', '', 'dashboard_delete')) { ?>
  ...
  <form action="<?= admin_url('ccx_dashboard/dashboards/update_dashboard')  ?>" method="post" class="form-horizontal">
  ...
  <!-- modal add widget -->
  <form id="formAddWidget" action="<?php echo base_url('admin/ccx_dashboard/dashboards/add_widget'); ?>" method="post" class="form-horizontal">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <div class="ccxdashboard-actions-wrap">
              <select id="changeCategory" class="form-control ccxdashboard-categories-select">
                <?php foreach ($categories as $category) { ?>
                  <option value="<?php echo $category['id']; ?>"><?php echo $category['name']; ?></option>
                <?php } ?>
              </select>
              &nbsp;
              <a href="<?php echo admin_url('ccx_dashboard/widgets'); ?>" class="btn btn-primary"><?= _l('view_all') ?></a>
            </div>

            <table class="table table-striped ccxdashboard-table">
              <thead>
                <th><?= _l('name') ?></th>
                <th></th>
                <th></th>
  ...
  <!-- modal staff -->
  <form action="<?= admin_url('ccx_dashboard/dashboards/update_staff')  ?>" method="post" class="form-horizontal">
    <div id="modalUpdateStaff" class="modal">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title"><?= _l('ccx_dashboard_staff_modal_title') ?></h5>
          </div>
          <div class="modal-body">
            <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
            ...
            <div class="row">
              <label class="control-label col-md-2"><?= _l('role') ?></label>
              <div class="col-md-8">
                <?php
                  $role_options = [];
                  if (isset($roles)) {
                  ...
                  echo render_select('ccx_dashboard_role_select', $role_options, array('roleid', 'name'), '', '', [], array(), '', '', false);
                ?>
              </div>
            </div>
            ...
            <div class="row mtop15">
              ...
              <div class="col-md-8">
                ...
                echo render_select('dashboard_staff[]', $staff, array('staffid', array('firstname', 'lastname')), '', $selected, array('id' => 'ccx_dashboard_staff_select', 'multiple' => true, 'data-actions-box' => true), array(), '', '', false);
                ?>
              </div>
            </div>
          </div>
  ...
  <!-- form delete -->
  <form id="formDelete" action="<?php echo base_url('admin/ccx_dashboard/dashboards/delete_dashboard'); ?>" method="post" class="d-none">
    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
    <input type="hidden" name="dashboard_id" value="<?= $dashboard['id'] ?>" />
  </form>
  <?php
    ...
    foreach ($staff as $member) {
      if (isset($member['role']) && $member['role']) {
        $role_staff_map[$member['role']][] = (string)$member['staffid'];
      }
    }
  ?>
  <script>
    ...
    const CCX_ROLE_STAFF_MAP = <?php echo json_encode($role_staff_map); ?>;
    $(function () {
      const $roleSelect = $('#ccx_dashboard_role_select');
      const $staffSelect = $('#ccx_dashboard_staff_select');

      function applyRoleSelection(roleId) {
        const staffIds = CCX_ROLE_STAFF_MAP[roleId] || [];
        $staffSelect.selectpicker('val', staffIds);
        $staffSelect.selectpicker('refresh');
      }

      $roleSelect.on('change', function () {
        applyRoleSelection($(this).val());
      });

      $('#modalUpdateStaff').on('shown.bs.modal', function () {
