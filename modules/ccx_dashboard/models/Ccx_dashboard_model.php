...
class Ccx_dashboard_model extends App_Model
{
    ...
    public function create_dashboard($dashboard_name, $note)
    {
        ...
        $this->db->insert(db_prefix() . 'ccx_dashboard_list', $data);
    ...
    {
        $dashboard_rows = $this->find_dashboard($clone_id);
    ...
    public function get_dashboards()
    {
        $sqlSelect = "
            SELECT TBLList.*
            FROM " .  db_prefix() . "ccx_dashboard_list TBLList
        ";
    ...
    {
        $sqlDeleteDashboard = "
            DELETE FROM " . db_prefix() . "ccx_dashboard_list WHERE id = " . $dashboard_id . "
        ";
        $sqlDeleteUsers = "
            DELETE FROM " . db_prefix() . "ccx_dashboard_users WHERE dashboard_id = " . $dashboard_id . "
        ";
    ...
    public function paginate_widgets($page)
    {
        $sqlCount = "
            SELECT 
                COUNT(*) AS TotalRows
            FROM " . db_prefix() . "ccx_dashboard_widgets TBLWidgets 
        ";
    ...
    public function get_widgets($filters)
    {
        $sqlSelect = "
            ...
            FROM " . db_prefix() . "ccx_dashboard_widgets TBLWidgets 
                LEFT JOIN " . db_prefix() . "ccx_dashboard_widget_categories TBLCategories
    ...
    public function update_dashboard_info($dashboard_id, $dashboard_name, $note)
    {
        ...
        $this->db->update(db_prefix() . 'ccx_dashboard_list', $data);
    }
    ...
    public function remove_widget($widget_name)
    {
        ...
        $sqlDeleteWidgets = "
            DELETE FROM " . db_prefix() . "ccx_dashboard_widgets WHERE widget_name = '" . $widget_name . "'
        ";
    ...
    public function update_widget($old_widget_name, $new_name, $new_category, $new_widget_name, $new_note)
    {
        ...
        $this->db->update(db_prefix() . 'ccx_dashboard_widgets', $data);
    }
    ...
    public function add_widget($new_name, $new_category, $new_widget_name, $new_note)
    {
        ...
        $this->db->insert(db_prefix() . 'ccx_dashboard_widgets', $data);
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'ccx_dashboard_widgets');
    }
    ...
    public function get_categories()
    {
        $sqlSelect = "
            SELECT 
                TBLCategories.*
            FROM " . db_prefix() . "ccx_dashboard_widget_categories TBLCategories ORDER BY TBLCategories.created_at DESC 
        ";
    ...
    public function store_category($name, $note)
    {
        ...
        $this->db->insert(db_prefix() . 'ccx_dashboard_widget_categories', $data);
    ...
    public function select_categories_by_ids($ids = [])
    {
        ...
        $sqlSelect = "
            SELECT 
                TBLCategories.*
            FROM " . db_prefix() . "ccx_dashboard_widget_categories TBLCategories 
    ...
    public function update_category_by_id($id, $name, $note)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'ccx_dashboard_widget_categories', $data);
    }

    public function destroy_category_by_id($id)
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'ccx_dashboard_widget_categories');
    }
    ...
    public function get_dashboard_staff($dashboard_id)
    {
        $sqlSelect = "
            SELECT 
                TBLUsers.* 
            FROM " . db_prefix() . "ccx_dashboard_users TBLUsers 
    ...
    public function update_dashboard_staff($dashboard_id, $dashboard_staff)
    {
        ...
        if (count($dashboard_staff) > 0) {
            $sqlInsert = "
                INSERT INTO " . db_prefix() . "ccx_dashboard_users (user_id, dashboard_id) VALUES 
            ";
            $countInsert = 0;
    ...
    public function get_dashboards_by_user($user_id)
    {
        $sqlSelect = "
            SELECT 
                TBLList.* 
            FROM " . db_prefix() . "ccx_dashboard_users TBLUsers 
                INNER JOIN " . db_prefix() . "ccx_dashboard_list TBLList 
