...
function ccx_dashboard_render_dashboard_widgets($container)
{
    $widgetsHtml = [];
...
function ccx_dashboard_render_widgets($widgets)
{
    $CI = &get_instance();

    foreach ($widgets as $widget) {
        echo '<div class="col-md-12">';
        echo $CI->load->view('ccx_dashboard/partials/widget_info', ['widget' => $widget], true);
        echo $CI->load->view('ccx_dashboard/widgets/' . $widget['widget_name'], [], true);
        echo '</div>';
    }
}

function ccx_dashboard_render_widgets_from_dashboard($dashboard, $container)
{
    $CI = &get_instance();
    $CI->load->model('ccx_dashboard_model');
    ...
    $widgets = $CI->ccx_dashboard_model->select_widgets_by_ids($ids);

    foreach ($widgets as $widget) {
        echo $CI->load->view('ccx_dashboard/widgets/' . $widget['widget_name'], [
            'widget' => $widget,
        ], true);
    }
}

function ccx_dashboard_get_available_widgets($dashboard)
{
    $CI = &get_instance();
    $CI->load->model('ccx_dashboard_model');
    ...
    $widgets = $CI->ccx_dashboard_model->select_widgets_except_ids($present_widgets);

    return $widgets;
}

function ccx_dashboard_get_categories() 
{
    $CI = &get_instance();
    $CI->load->model('ccx_dashboard_model');

    $categories = $CI->ccx_dashboard_model->get_categories();

    return $categories;
}

function ccx_dashboard_scan_widgets_2()
{
    ...
    if ($widgets) {
        ...
        foreach ($widgets as $widget_name) {
            ...
            $header = ccx_dashboard_get_headers_widget($path);
...
function ccx_dashboard_scan_widgets()
{
    $CI = &get_instance();
    $CI->load->model('ccx_dashboard_model');
    ...
    if ($widgets) {
        ...
        $db_widgets = $CI->ccx_dashboard_model->select_widgets_except_names();
...
function ccx_dashboard_scan_widgets_with_activation($active)
{
    $data = ccx_dashboard_scan_widgets();
    return array_filter($data, function ($v) use ($active) {
        return $v['active'] == $active;
    });
}

function ccx_dashboard_get_headers_widget($widget_path)
{
...
function get_widgest_folder_path(){
    ...
    return APP_MODULES_PATH.CCX_DASHBOARD_MODULE_NAME.'/views/widgets';
