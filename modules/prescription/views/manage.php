                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <!-- Tabs -->
                                <div class="prescription-tabs-container">
                                    <a href="<?php echo admin_url('prescription?status=all'); ?>"
                                        class="prescription-tab <?php echo ($filters['status'] == 'all' ? 'active' : ''); ?>">
                                        All
                                    </a>

                                    <a href="<?php echo admin_url('prescription?status=Processing'); ?>"
                                        class="prescription-tab <?php echo ($filters['status'] == 'Processing' ? 'active' : ''); ?>">
                                        Processing
                                        <?php if (isset($status_counts['Processing']) && $status_counts['Processing'] > 0) { ?>
                                            <span
                                                class="prescription-badge bg-blue"><?php echo $status_counts['Processing']; ?></span>
                                        <?php } ?>
                                    </a>

                                    <a href="<?php echo admin_url('prescription?status=Emergency'); ?>"
                                        class="prescription-tab <?php echo ($filters['status'] == 'Emergency' ? 'active' : ''); ?>">
                                        Emergency
                                        <?php if (isset($status_counts['Emergency']) && $status_counts['Emergency'] > 0) { ?>
                                            <span
                                                class="prescription-badge bg-red"><?php echo $status_counts['Emergency']; ?></span>
                                        <?php } ?>
                                    </a>

                                    <a href="<?php echo admin_url('prescription?status=Completed'); ?>"
                                        class="prescription-tab <?php echo ($filters['status'] == 'Completed' ? 'active' : ''); ?>">
                                        Completed
                                        <?php if (isset($status_counts['Completed']) && $status_counts['Completed'] > 0) { ?>
                                            <span
                                                class="prescription-badge bg-green"><?php echo $status_counts['Completed']; ?></span>
                                        <?php } ?>
                                    </a>
                            ...
                            <div class="col-md-4 text-right">
                                <!-- Filters -->
                                <form action="<?php echo admin_url('prescription'); ?>" method="GET" class="form-inline"
                                    style="display: flex; align-items: center; justify-content: flex-end; gap: 10px;">
                                    <input type="hidden" name="status" value="<?php echo $filters['status']; ?>">
                                    <?php if (get_option('prescription_show_date_filter') == '1' || get_option('prescription_show_date_filter') === '') { ?>
                            ...
                            <table class="table table-bordered dt-table" data-order-col="0" data-order-type="desc">
                                <thead>
                                    <tr>
                                        ...
                                        <?php $mode = get_option('prescription_mode_type'); ?>
                                        ...
                                        <?php if ($mode == 'manual') { ?>
                                            <th>Manual Rx</th>
                                        <?php } ?>
                                        <?php if (get_option('prescription_show_options_column') == '1' || get_option('prescription_show_options_column') === '') { ?>
                                            <th>Options</th>
                                        <?php } ?>
                                    </tr>
                                </thead>
                                                ...
                                                <td class="text-center">
                                                    <?php if (!empty($row['prescription_id'])) {
                                                        ...
                                                        if ($row['status'] == 'Completed') {
                                                            $completed_date = !empty($row['prescription_completed_date']) ? $row['prescription_completed_date'] : $row['prescription_date'];

                                                            // Get Timeout Setting (Minutes to Seconds)
                                                            $timeout_min = get_option('prescription_edit_timeout');
                                                        ...
                                                        if ($can_edit) {
                                                            ?>
                                                            <a href="<?php echo admin_url('prescription/edit/' . $row['prescription_id']); ?>"
                                                        ...
                                                        <a href="<?php echo admin_url('prescription/view/' . $row['prescription_id']); ?>"
                                                            class="btn btn-default btn-icon" title="View Prescription"><i
                                                                class="fa fa-eye"></i></a>
                                                        <?php
                                                    } else { ?>
                                                        <a href="<?php echo admin_url('prescription/create/' . $row['id']); ?>"
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-right">
                                                            <?php if (!empty($row['prescription_id'])) { ?>
                                                                <li><a href="#" class="action-recreate"
                                                                        data-id="<?php echo $row['prescription_id']; ?>">Re-Create</a>
                                                                </li>
                                                            <?php } ?>
                                                            <?php if ($row['status'] != 'Cancel') { ?>
                                                                <li><a href="<?php echo admin_url('prescription/cancel/' . $row['id']); ?>"
                                                                        class="action-cancel"
                                                            ...
                                                            <?php if ($row['status'] == 'Cancel') { ?>
                                                                <li><a href="<?php echo admin_url('prescription/uncancel/' . $row['id']); ?>"
...
<a href="<?php echo admin_url('prescription/settings'); ?>" class="btn btn-info btn-icon" title="Settings"
    ...
    $(function () {
        ...
        $('.btn-manual-rx').on('click', function () {
            ...
            if (confirm('Are you sure you want to mark this as Completed?')) {
                $.post(admin_url + 'prescription/change_status/' + id + '/Completed', function (response) {
        ...
        $('.action-recreate').on('click', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            if (confirm('Are you sure you want to delete this prescription and create a new one? This action cannot be undone.')) {
                window.location.href = admin_url + 'prescription/recreate/' + id;
            }
        });
    });
</script>