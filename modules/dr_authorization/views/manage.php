...
<div id="wrapper">
    <div class="content">
        ...
        <a href="<?php echo admin_url('dr_authorization/settings'); ?>" class="btn btn-info btn-icon"
                        ...
                        <div class="transcriptor-controls-wrapper"
                            ...
                            <div class="transcriptor-filters" style="margin-bottom: 0;">
                                ...
                                foreach ($statuses as $s) {
                                    ...
                                    $params = $_GET;
                                    $params['status'] = $s;
                                    $url = admin_url('dr_authorization?' . http_build_query($params));
                            ...
                            <form method="get" action="<?php echo admin_url('dr_authorization'); ?>">
                                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                                    <?php
                                    $show_date = get_option('dr_authorization_show_date_filter') != '0';
                                    $show_user = get_option('dr_authorization_show_user_filter') != '0';
                                    $show_dept = get_option('dr_authorization_show_department_filter') != '0';
                                    $show_ref = get_option('dr_authorization_show_ref_doctor_filter') != '0';
                                    ?>
                                    <?php if ($show_date) { ?>
                                    ...
                                    if (($show_date || $show_user || $show_dept || $show_ref) && $has_filters) {
                                        ?>
                                        <div>
                                            <a href="<?php echo admin_url('dr_authorization'); ?>"
                        ...
                        <table class="table dt-table" data-order-col="0" data-order-type="asc">
                            ...
                            <tbody>
                                <?php foreach ($requests as $key => $r) {
                                    ...
                                    $payment_text = '';
                                    if (get_option('dr_authorization_show_payment_info') == '1') {
...
<script>
    function open_report(test_id) {
        $.post(admin_url + 'dr_authorization/create_report/' + test_id).done(function (response) {
            response = JSON.parse(response);
            if (response.success) {
                $.get(admin_url + 'dr_authorization/edit_report/' + response.id).done(function (html) {
    ...
    function view_report(id) {
        $.get(admin_url + 'dr_authorization/view_report/' + id).done(function (html) {
            $('#modal_wrapper').html(html);
