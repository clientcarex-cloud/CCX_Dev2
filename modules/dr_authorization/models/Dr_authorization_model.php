...
class Dr_authorization_model extends App_Model
{
    ...
    public function get_transcription_requests($filters = [])
    {
        ...
        // STRICT FILTER: Status 8 (Authorization Required) ONLY
        $this->db->where(db_prefix() . 'patient_tests.status', 8);

        // Apply Status Filter (Tabs) - Still allowing if user wants to sub-filter or if we want to keep structure,
        // but essentially we are already filtering by 8. 
        // If the tabs (Regular, Emergency) are clicked, they might return 0 results if they check for other statuses.
        // We should probably rely on the tabs to filter WITHIN the 'Authorization Required' bucket, e.g. Emergency + Auth Required?
        // But for now, user asked "show data only with status of id 8".
        // Let's modify the tabs logic to just handle Emergency vs Regular within the status 8 subset if needed.
        // For simplicity and matching request, I'll enforce status 8 and optionally respect Emergency flag if passed.
