...
<aside id="menu" class="sidebar">
    ...
    <div class="dropdown sidebar-user-profile tw-mt-[80px] tw-mx-1.5 ">
        ...
        <ul class="dropdown-menu tw-w-full">
            <li class="header-my-profile"><a href="<?= admin_url('profile'); ?>"><?= _l('nav_my_profile'); ?></a>
            </li>
            ...
            <?php if (!is_language_disabled()) { ?>
                <li class="dropdown-submenu pull-left header-languages">
                    <a href="#" tabindex="-1"><?= _l('language'); ?></a>
            ...
            <li class="header-logout">
                <a href="#" onclick="logout(); return false;"><?= _l('nav_logout'); ?></a>
            </li>
        </ul>
    </div>
    <ul class="nav metis-menu tw-mt-[15px]" id="side-menu">
        ...
        <?php foreach ($sidebar_menu as $key => $item) {
            ...
            <li class="menu-item-<?= e($item['slug']); ?> <?= isset($item['li_attributes']) && isset($item['li_attributes']['class']) ? $item['li_attributes']['class'] : ''; ?>"
                <?= _attributes_to_string($item['li_attributes'] ?? []); ?>>

                <?php if (isset($item['li_attributes']) && isset($item['li_attributes']['class']) && strpos($item['li_attributes']['class'], 'menu-section-label') !== false) { ?>
                <div class="menu-section-heading" onclick="
                    // Simple inline script to toggle visibility of next items
                    var next = this.parentElement.nextElementSibling;
                    var isCollapsed = this.parentElement.classList.toggle('collapsed');
                    localStorage.setItem('menus_sections_collapsed_' + '<?= $item['slug'] ?>', isCollapsed);
                    ...
                    return false;
                ">
                    <span class="menu-text"><?= e(_l($item['name'], '', false)); ?></span>
                     <i class="fa fa-chevron-down section-arrow" style="float:right; font-size:12px; transition: transform 0.3s;"></i>
                </div>
                
                <!-- Init state script -->
                <script>
                    (function() {
                        var isCollapsed = localStorage.getItem('menus_sections_collapsed_' + '<?= $item['slug'] ?>') === 'true';
                        if (isCollapsed) {
                             document.write('<style>.menu-item-<?= $item['slug'] ?> .section-arrow { transform: rotate(-90deg); }</style>');
                        }
                    })();
                </script>
                ...
                <?php } else { ?>
                    ...
                    <a href="<?= count($item['children']) > 0 ? '#' : $item['href']; ?>" aria-expanded="false"
                    ...
                    <ul class="nav nav-second-level collapse" aria-expanded="false">
                        <?php foreach ($item['children'] as $submenu) { ?>
                            <li class="sub-menu-item-<?= e($submenu['slug']); ?>" <?= _attributes_to_string($submenu['li_attributes'] ?? []); ?>>
                                <a href="<?= e($submenu['href']); ?>" <?= _attributes_to_string($submenu['href_attributes'] ?? []); ?>>
            ...
            <?php
            // Inject script to hide this item if previous section was collapsed
            if (!isset($item['li_attributes']) || strpos(($item['li_attributes']['class'] ?? ''), 'menu-section-label') === false) {
                // This is a normal item
                ?>
                <script>
                    (function () {
                        // Find previous section
                        // This is tricky doing it item by item. 
                        // Better to run a single pass at the bottom or top of the page.
                    })();
                </script>
            <?php } ?>
